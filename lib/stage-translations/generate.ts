...
export async function generateStageTranslation(input: {
  ...
  const pageData = await loadStageTranslationPageData(input.projectId, input.stageId, userId, input.responseId)
  ...
  try {
    ...
    await audit({
      ...
      metadata: { stageId: pageData.stage.id, responseId: pageData.response.id },
    })
