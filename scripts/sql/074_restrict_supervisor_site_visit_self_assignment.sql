-- 074_restrict_supervisor_site_visit_self_assignment.sql
-- Restrict regular supervisors to assigning only themselves when scheduling
-- direct Site Visits or approving Client Visit Requests. Admins and Project Admins
-- retain the ability to select and assign any valid project supervisor.
...
begin
  ...
  if not is_admin then
    if coalesce(assigned_user_ids, '{}'::uuid[]) <> array[actor_id] then
      raise exception 'Supervisors can only schedule visits for themselves';
    end if;
  end if;
