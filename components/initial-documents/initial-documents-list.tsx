...
export function InitialDocumentsList({
  ...
  const [category, setCategory] = useState<"all" | InitialDocumentCategory>("all")
  ...
  }, [category, documents, locale, search, sort])

  const drawingCount = documents.filter((item) => item.category === "approved_drawings").length
  const contractCount = documents.filter((item) => ["contract", "consultant_agreement", "contractor_agreement"].includes(item.category)).length
  const otherCount = documents.filter((item) => item.category === "other").length
        ...
        <>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            <div className="relative min-w-0 flex-1 sm:max-w-md">
              ...
              <Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder={copy.search} className="h-10 rounded-xl ps-9" />
            </div>
            <Select value={category} onValueChange={(value) => setCategory(value as typeof category)}>
              <SelectTrigger className="h-10 w-full rounded-xl px-3 sm:w-56">
                <SelectValue>{(value) => value === "all" ? copy.allCategories : (isArabic ? getInitialDocumentCategory(value).labelAr : getInitialDocumentCategory(value).label)}</SelectValue>
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">{copy.allCategories}</SelectItem>
                {INITIAL_DOCUMENT_CATEGORIES.map((item) => <SelectItem key={item.value} value={item.value}>{isArabic ? item.labelAr : item.label}</SelectItem>)}
              </SelectContent>
            </Select>
            <Select value={sort} onValueChange={(value) => setSort(value as SortValue)}>
              <SelectTrigger className="h-10 w-full rounded-xl px-3 sm:w-44">
                <SelectValue>{(value) => ({ newest: copy.newest, oldest: copy.oldest, file_name: copy.fileName, project_name: copy.projectName, file_size: copy.fileSize }[value as SortValue])}</SelectValue>
              </SelectTrigger>
              <SelectContent>
            ...
            <>
              ...
              <div className="grid gap-3 md:hidden">
                {filtered.map((item) => {
                  ...
                  const categoryLabel = isArabic ? getInitialDocumentCategory(item.category).labelAr : getInitialDocumentCategory(item.category).label
                  return (
      ...
      <Dialog open={Boolean(preview)} onOpenChange={(open) => { if (!open) setPreview(null) }}>
        <DialogContent className="h-[min(88vh,900px)] max-w-[min(94vw,1100px)] grid-rows-[auto_1fr] overflow-hidden p-4 sm:max-w-[min(94vw,1100px)]">
          <DialogHeader className="pe-10">
            ...
            <DialogDescription>{preview ? (isArabic ? getInitialDocumentCategory(preview.category).labelAr : getInitialDocumentCategory(preview.category).label) : ""}</DialogDescription>
          </DialogHeader>
          {preview ? (
...
function DocumentRow({ item, showProject, locale, copy, onPreview }: {
  ...
  return (
    <tr className="hover:bg-muted/20">
      ...
      <td className="whitespace-nowrap px-4 py-3.5"><span className="rounded-md bg-muted px-2 py-1 text-xs font-medium">{isArabic ? category.labelAr : category.label}</span></td>
