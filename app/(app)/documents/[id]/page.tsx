...
import { ArrowLeft, CalendarDays, CheckCircle2, FileText, FolderLock } from "lucide-react"
...
export default async function DocumentDetailsPage({
  ...
  return (
    <div className="mx-auto flex w-full max-w-5xl flex-col gap-5">
      ...
      {query.created === "draft" || query.created === "published" ? (
        <div className="flex items-center gap-3 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-medium text-emerald-800 dark:border-emerald-900 dark:bg-emerald-950/40 dark:text-emerald-200">
          ...
          {query.created === "published" ? "Document published successfully." : "Draft saved successfully."}
        </div>
      ) : null}
      ...
      <Card className="gap-0 overflow-hidden py-0">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div className="min-w-0">
              <div className="mb-2 flex flex-wrap items-center gap-2 text-xs font-semibold uppercase tracking-[0.12em] text-blue-200">
                ...
                <span>{typeLabel[document.document_type] ?? "Document"}</span>
              </div>
        ...
        <CardContent className="grid gap-4 border-b bg-muted/25 px-6 py-4 text-sm sm:grid-cols-3">
